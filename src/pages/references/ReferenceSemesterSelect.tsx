...
export default function ReferenceSemesterSelect() {
  return (
    <div className="min-h-screen bg-background">
      <Header />
      <PageContainer 
        title="Study References" 
        subtitle="Select a semester to browse reference books and materials"
      >
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-4">
          {semesters.map((semester, index) => (
            <Link key={semester} to={`/references/semester/${semester}`}>
              <MotionCard
                variant="interactive"
                delay={index * 0.05}

...
export default function QuizSubjectSelect() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <Header />
      <PageContainer 
        title={`Semester ${semesterNum} Quizzes`}
        subtitle={`Select a subject to practice • ${user?.branch}`}
      >
        <div className="grid gap-4 sm:grid-cols-2">
          {subjects.map((subject, index) => (
            <Link 
              key={subject.id} 
              to={`/quiz/semester/${semester}/subject/${subject.id}`}
            >
              <MotionCard
                variant="interactive"
                delay={index * 0.05}
